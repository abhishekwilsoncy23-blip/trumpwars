const Level3 = (function() {
    ...
    let mibDirection = 1; let mibSpeed = 1.5; let phase = 1; 
    ...
    return {
        ...
        update: function() {
            ...
            if (phase === 1) {
                ...
                mibs.forEach(mib => {
                    ...
                    if (Math.random() < 0.01) mibBullets.push({ x: mib.x + mib.width/2 - 4, y: mib.y + mib.height, width: 8, height: 15, speed: 5 });
                });

                if (hitEdge) {
                    mibDirection *= -1;
                    mibs.forEach(mib => mib.y += 20); 
                }

                bullets.forEach((b, bIndex) => {
                    mibs.forEach((mib, mIndex) => {
                        if (checkCollision(b, mib)) { bullets.splice(bIndex, 1); mibs.splice(mIndex, 1); }
                    });
                });
                ...
                mibs.forEach(mib => { if (mib.y + mib.height >= player.y) triggerGameOver("The Men in Black got you!", () => changeLevel(Level3)); });

                if (mibs.length === 0) { phase = 2; document.getElementById("btnAction").innerText = "CATCH!"; }

                alien.x += alien.speed * alien.direction;
                if (alien.x <= 0 || alien.x + alien.width >= canvas.width) alien.direction *= -1;

            } else if (phase === 2) {
                alien.y += alien.speed + 2; 
                alien.x += alien.speed * alien.direction;
                if (alien.x <= 0 || alien.x + alien.width >= canvas.width) alien.direction *= -1;

                if (checkCollision(player, alien)) {
                    triggerVictory("AREA 51 RAID SUCCESS!<br>Alien Secured.", null); 
        ...
        draw: function(ctx) {
            ...
            if (phase === 1) {
                ...
                ctx.fillText("Shoot the MiBs! DON'T shoot the Alien!", 50, 580);
            } else {
